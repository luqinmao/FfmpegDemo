...
import com.lansoeditor.demo.helper.VideoFocusView;
import com.lansoeditor.demo.helper.VideoPreviewView;
import com.lansoeditor.demo.helper.VideoProgressView;
...
public class VideoRecordActivity extends Activity implements Handler.Callback ,View.OnClickListener {
    private final static String TAG = "SegmentRecorderActivity";


    public static final float MAX_RECORD_TIME = 60 * 1000f;  //设置录制的最大时间.  15秒.
    ...
    private boolean isMixVideo = false;
    private boolean isFirst;
    private boolean isPPP = false; //从ppp界面过来的
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isMixVideo = getIntent().getBooleanExtra("isMixVideo",false);
        isFirst =  getIntent().getBooleanExtra("isFirst",true);
        isPPP =  getIntent().getBooleanExtra("isPPP",true);
...
//		segmentRecorder = new OpenSegmentsRecorder(this, cameraTextureView.getHolder(),1088,1920,3000*1000);
    ...
    private void recordEnd() {
        ...
        if(dstfile!=null){  //文件存在.
            if (isMixVideo){
                if (isFirst){
                    PrefUtils.setString(this,"video1",dstfile);
                }else{
                    PrefUtils.setString(this,"video2",dstfile);
                }
                Intent intent=new Intent(VideoRecordActivity.this,VideoPlayTestActivity.class);
                intent.putExtra("videopath", dstfile);
                startActivity(intent);
            }else{
                if (isPPP){
                    Intent intent=new Intent(VideoRecordActivity.this,PPPActivity.class);
                    intent.putExtra("videopath", dstfile);
                    setResult(RECORD_VIDEO, intent);
                    finish();

                }else{
                    VideoEditActivity.start(VideoRecordActivity.this,dstfile);
                }
            }

        } else {
            Toast.makeText(VideoRecordActivity.this, "视频合成失败", Toast.LENGTH_LONG).show();
        }
    }
    @Override
...
//----------------follow is ui code----------一下是UI界面代码--------------------------------------------------------------------------------------------------------
    ...
    private void initView()
    {
        ...
        recorderVideoBtn.setOnTouchListener(new View.OnTouchListener() {
    ...
    private Rect doTouchFocus(float x, float y) {
        ...
        try {
            ...
            Camera.Area focusA = new Camera.Area(targetFocusRect, 1000);
